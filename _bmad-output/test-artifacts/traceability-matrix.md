...
| SYS-E2E-002 | E2E | `e2e/dashboard-run-gate-artifacts.spec.ts`, `e2e/gate-reject-retry-edit-concurrency.spec.ts` | 2 | built | ✅ Built (2026-07-02, `bmad-qa-generate-e2e-tests`) — Journey 1: dashboard → SCP search/select → create run → SSE-observed gate_pending per stage → approve all 5 gates → per-stage artifact panel (text/images/audio/subtitle/video) → completion verified via `GET /runs/{id}`. Journey 2 (2026-07-03): gate reject (implicit retry loop) + explicit `재시도` retry trigger with B-1 concurrency-guard probes (API 409 + UI control hiding) + scenario/subtitle inline edit + re-approve + downstream (video) rerun confirmation. Found and documented (not fixed, out of scope for this skill) four real gaps: no SSE event for the run→`complete` transition, no SPA history-fallback for direct navigation to `/app/runs/{id}`, no frontend-level concurrency guard beyond the retried stage's own controls (server-side B-1 guard is correctly enforced), and a possible TOCTOU window in `retry_stage`/`edit_artifact`'s B-1 guard under genuinely concurrent requests — see `_bmad-output/implementation-artifacts/tests/test-summary.md`. |
...
| SYS-E2E-002 | P2 | R-005 | E2E | **FULL** | `e2e/dashboard-run-gate-artifacts.spec.ts` (built 2026-07-02), `e2e/gate-reject-retry-edit-concurrency.spec.ts` (built 2026-07-03) |
