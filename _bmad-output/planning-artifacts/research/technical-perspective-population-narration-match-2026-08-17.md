...
| ⑤ 배경 인구 | ~~**고쳐졌으나 꺼진 채로 출하됐다**~~ → **2026-08-22 Story 14.4로 출하됨** | 10.2가 가드를 만들고 코드 기본값 `0`으로 출하 → 15일간 `never screened`. 2026-08-17 첫 발화에서 오염 3→0, never screened 43→0. ~~지금도 `.env` 핀 상태~~ → **코드 기본값이 `2`가 되고 `.env` 핀은 삭제됐다**(14.4). 재실측: 43샷 중 rung 0=38 / rung 1=5 / 소진 0 — 다만 그 5건은 **탐지기 히트**이고 확인된 오염은 3건이다(`14-4-live-validation/`) |
...
- 남은 리서치 질문은 하나: **"그림 속 인물"(액자·모니터·포스터 안)을 어느 층이 책임지나.**
  ...
  **✅ 답 (Story 14.4, 2026-08-22): 소관은 14.1의 승인 게이트다.** 액자 속 인물은 플레이트
  단위 속성이므로 사람이 승인 시점에 한 번 판정하면 되고, 14.1의 게이트는 이미 ③⑤⑦의 단일
  강제 지점이다. **런타임 가드 확장은 기각** — (i) 가드가 좁혀진 목적(합성 카드 ↔ 프레임 속
  **몸**의 충돌)을 깨고, (ii) 위에 인용한 그 구분을 정확히 다시 열고, (iii) 중복 인물이 아닌
  결함에 히트마다 렌더 한 장을 영구 지불하는데 사람은 플레이트당 한 번만 답하면 된다.
  `vision_check.CHECK_PROMPT`의 FALSE 목록은 오류가 아니라 **다른 질문에 대한 정답**이고,
  `CHECK_PROMPT`는 규칙 하나를 유지한다(`tests/services/test_vision_check.py:127`/`:135`).
  **잔여 갭은 감수 리스크로 명시**: 자유생성 샷(run `4b35c0ed` 12/43)은 승인 게이트에 도달하지
  않으므로 14.1의 세트가 덮기 전까지 이 부류는 커버되지 않는다. 위 첫 항목이 이미 반증한 대로
  부정 프롬프트는 대안이 아니다.

  **부수 결정 (같은 스토리)**: (1) 출하 규율 쪽이 먼저라는 §2의 진단대로
  `background_person_guard_attempts`는 코드 기본값 `2`가 됐고 `.env` 핀은 삭제됐다
  (`14-4-live-validation/`: 43샷 rung 0 38 / rung 1 5 / 소진 0, 비전 콜 1.46~2.58초).
  (2) 판정 불가(undecidable)는 **수용하고 단을 소비하지 않되** 샷 단위로 경고한다
  (`reason=detector_undecidable_shot`) — 이 런의 판정 불가 1건이 경고 0건이었고, 미검사
  프레임이 검증된 프레임과 구분되지 않는 것이 13.1이 없애려던 바로 그 결함이다.
  (3) 탐지기가 이미 돌려주던 `notes` 문장을 로그로 남긴다 — `S00201`의 액자 속 애니풍 인물이
  그 노트에 있었는데 버려서 n=1이었다. 14.1이 쓸 코퍼스가 이제 공짜로 쌓인다.
...
- **Patcher: Repairing Catastrophic-Neglect via Attention-Guided Feature Enhancement**
  ...
  **어느 명제가 깨졌는지**를 내놓는다. Patcher의 ①단계 입력이 우리에게 이미 있는 셈이다.
  ...
  > **⚠️ 2026-08-22 — 이 "왜 맞나"는 철회한다(원문은 기록으로 보존).** §4-5가 같은 날 실측으로
  > 반증했다: DSG 명제는 `image_prompt`에서 분해되므로 질문이 답을 품은 **유도질문**이고,
  > 사건 축(`state`)은 ρ=−0.174로 **판독불가 프레임에서 오히려 높다**(0.6250 vs 0.3895).
  > Patcher ①단계("어느 객체가 누락됐나")는 그 DSG와 **같은 설계**(프롬프트에서 도출한 예/아니오를
  > VLM에 묻기)이므로, "우리에게 이미 있다"던 그 입력은 **가장 필요한 프레임에서 방향이 뒤집혀
  > 있다**. 샘플러 개입의 비용을 상쇄할 자산이 아니었다 — §4-3의 결론(프롬프트 재작성 우선)은
  > 비용 비대칭뿐 아니라 **이 이유로도** 선다. 14.5는 이 문장을 근거로 인용하지 말 것.
...
3. **Patcher/Attend-and-Excite가 우리 ComfyUI 경로에서 구현 가능한가** — 커스텀 노드 필요 여부.
   ...
   >
   > **재검증 2026-08-22(2차 세션)**: 스냅숏이 아니라 **라이브 설치**를 다시 확인했다 —
   > `$HOME/workspaces/ComfyUI/custom_nodes` 실물 10종이 `env-snapshot.json`과 일치하고 코어
   > 해시도 `f350a842`로 동일, 훅 3종(`model_patcher.py:442` `set_model_attn1_patch`, `:445`
   > `set_model_attn2_patch`, `:457` `set_model_attn2_output_patch`) 실재 확인. 결론 불변.
   >
   > **그리고 §3이 이 분기에 붙여 둔 "우리가 유리한 점"은 철회됐다** — Patcher ①단계 입력을
   > 13.2의 DSG가 공짜로 준다는 논거는 §4-5가 그 DSG를 **뒤집힌 축**으로 실측해 무효화했다
   > (§3의 해당 불릿 주석 참조). 샘플러 개입은 비용만 비싼 것이 아니라 **1단계 입력도 없다**.
   >
...
4. **`camera_angle` 필드와 `image_prompt` 본문의 앵글 서술이 충돌한다**(본문이 필드를 덮어쓴다:
   ...
   **먼저 확인할 것.**
   ...
   >
   > **✅ 판별 완료 — 2026-08-22, 신규 스토리 없이 실행(GPU 0, `src` 변경 0). (a) 기각, (b) 미확정,
   > 그리고 두 가설의 공통 전제가 대조군에서 깨졌다.** 출하 플레이트 43장을 사전등록한 투영기하 규칙
   > 하나(소실점 세로 위치 `y_h`; 수평 카메라면 0.5)로 프롬프트 비열람 판정 →
   > **부감 17 / 눈높이 20 / 앙각 4 / 판독불가 2**.
   >
   > - **(a) 내용 질량 — 반증(방향이 반대).** 표면 어휘 히트 상위군 부감율 **28%**(7/25) vs 하위군
   >   **62%**(10/16), p=0.0505. (a)의 예측은 정확히 반대였다. 남은 역방향 신호도 대부분 **프롬프트 길이**
   >   교란이다 — 총 단어수만으로 갈라도 같은 세기(p=0.0624)이고, 100단어당 정규화하면 p=0.1180.
   > - **(b) 조명 어휘 — 기각도 확정도 아님.** 히트≥1 부감율 **63%**(12/19) vs 무히트 **23%**(5/22),
   >   p=0.0124로 주효과는 있다. 그러나 ① 용량-반응이 없고(0히트 23% → 1히트 69% → **2히트 33%**),
   >   ② **최대 용량 3샷이 판정 셋을 하나씩 낸다**(`S00100` 부감 / `S00103` **앙각** / `S00901` 눈높이) —
   >   즉 위에서 결정적이라고 지목한 두 샷이 **둘 다 (b)에 반한다**, ③ (i)<3 층 안에서 효과가 사라진다(p=0.61).
   > - **민감도**: 경계 ±0.05 판정 13건을 빼면(n=28) 두 축 모두 유의성을 잃는다((i) p=0.45, (ii) p=0.25).
   > - **대조군(이 문서가 없다고 적었던 것) — 시점은 프롬프트 텍스트의 함수가 아니다.** 10.2 가드가
   >   재렌더한 5샷은 **시드만 올리고 `image_prompt`는 그대로**다(`image.py` `_seed_ladder`) = 같은 프롬프트
   >   두 번 뽑기. **5쌍 중 2쌍에서 시점 범주가 뒤집혔다**(`S00202` 앙각→눈높이, `S00301` 눈높이→**부감**).
   >   뒤집힌 `S00301`은 위 (b) 표에서 부감으로 세어진 샷이다. **텍스트 가설의 검정력 천장이 여기다.**
   >
   > **대신 확정된 것 — ②는 n=1이 아니다.** 부감 17건 중 필드가 `high-angle`인 것은 6뿐 →
   > **요청하지 않은 부감 11건**(경계 판정 전부 빼도 7건). `high-angle`은 **6/6 지켜지고**,
   > `wide`는 1/9만 부감이며 `medium`은 **6/10**이 부감이다. 즉 모델이 부감 지시를 못 따르는 게 아니라
   > **`medium`에 수직 앵커가 없다**는 쪽이 그림에 맞는다(사후·탐색적, p=0.0413 / 경계 제외 시 0.27 —
   > **확정 아님**, 14.2가 새 런에서 확인할 후보). 기각된 사후 축도 기록: 슬롯-1의 수평면 명사(p=1.0),
   > 슬롯-1 절 길이(p=1.0).
   >
   > **14.2 인계의 실질**: ① 바닥/천장 질량 게이트는 만들지 마라(방향이 반대), ② 시점은 렌더 전 텍스트
   > 스크리닝으로 보장할 수 없다(같은 프롬프트 40% 뒤집힘) — 이 부류에 이 프로젝트에서 작동한 유일한
   > 수단은 **탐지 후 재생성**이고(§2가 부정 프롬프트에 대해 같은 결론을 이미 적었다) 시드 래더는 이미 있다,
   > ③ 검정 목표는 **요청하지 않은 부감 11/35**를 줄이는 것이고 `high-angle` 6/6·`wide` 8/9가 회귀 감시선이다.
   > 근거·재산출: `implementation-artifacts/14-0-angle-conflict/report.md` §8,
   > `PREREGISTRATION-4-4-hypotheses.md`, `discriminate_viewpoint_hypotheses.py`,
   > `build_viewpoint_sheets.py`, `viewpoint_verdicts{,_reseed}.csv`, `sheet_*.jpg`.
   >
   > **부수 확인**: 이 문서 §0의 "오염 3→0"과 달리 가드 재렌더는 **5샷**이다(픽셀 대조; 나머지 38장은
   > PNG 재인코딩으로 바이트만 다르다). 14.4가 가드 계수를 만질 때 확인할 것.
...
1. ~~**§4-4 앵글 충돌 확인** — 코드 읽기, GPU 0, 문헌 0. ②의 절반이 여기서 끝날 수 있다.~~
   **✅ 완료(2026-08-21, Story 14.0 §4-4) — 그리고 반증이다.** 충돌 0건(43/43 일치)이므로 ②의 절반은
   ...
   (b) 조명 어휘 오독)을 동등하게 열어 둔 것**이다. → **다음 순번은 3번**
   (2번은 2026-08-22에 닫혔다).
2. ~~**14.4 가드 코드 기본값 승격** — 이미 실측됨(3→0), 결정만 남았다. ⑤가 여기서 닫힌다.~~
   **✅ 완료(2026-08-22, Story 14.4).** 코드 기본값 `0 → 2`, `.env` 핀 삭제, `.env.example`의
   잠재 되돌림 4건 주석 처리. ⑤의 **출하 규율 절반은 닫혔다**. 남은 절반은 "그림 속 인물"이고
   그것은 가드 확장이 아니라 **14.1 승인 게이트** 소관으로 결정됐다(§2 참조). 부수 산출:
   13.6의 표류 리포트(`scripts/report_decision_drift.py`)와 단발 판정불가의 샷 단위 경고.
3. **14.7 scenario 리뷰어 정합** — 비-GPU, 스테일 규칙 2건. ← **현재 최우선**
...
**2026-08-22: 14.4도 닫혔다** — 남은 최우선 순번은 **14.7**(비-GPU, 스테일 규칙 2건)이고
그 다음이 **14.1 승인 세트**다(14.2는 14.1 선행).
</content>
</invoke>
