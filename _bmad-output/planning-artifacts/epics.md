...
**2026-08-30 종결 — 축을 갈아끼우고 플래그를 켰다(GPU 0·VLM 0·렌더 0).** Phase 1의 심사 기준은 `(b) 재현 오차 < (c) 허용 폭` 하나였고 통과한 후보는 **② 시점을 안 쓰는 축**뿐이다(`location_key` + people-free + 어포던스, `viewpoint`는 선택에서 안 읽음). ①은 커밋된 구현이 없고 임시 판본이 같은 이미지에 **0.93/0.97/0.41**로 발산(민감도 0.56 > 범주 폭 0.20), ③은 스코어러도 임계값도 없어 (c) 미정의(8.19 임베딩층 부재를 이번 세션에 직접 재확인), ④는 판정자 간 오차 0.072가 **키 내부 최소 간격 중앙값 0.010**보다 커서 수요 6키 전부에서 랭킹이 노이즈다. ②의 (b)=0은 **측정을 없애서** 얻은 0이다(닫힌 14값 enum의 문자열 동등성). **2-경로 검증 P1 = 1/42 = 2.4% < 밴드 5.0% PASS**(밴드는 `d797a8a`에 측정 전 커밋). **커버리지 C1′ PASS 6/6키 · C2′ PASS · C3′ PASS 24/24 = 100.0%** — 옛 축 대조군은 같은 명령이 한 화면에 찍는다(**C1 FAIL 5/10 · C3 FAIL 17/24 = 70.8%**, 14.1 커밋 수치를 그대로 재생하므로 대조군이 유효하다). servable 분모 **24**와 `C3_MIN_SHARE=0.90`은 한 글자도 안 바꿨다. **⚠️ C3′의 공허성은 측정 전에 공시됐다**(사전등록 §3: 축 ②에서 실패 불가) — PASS로 즐기지 않고 그렇게 보고한다. 하중은 C1′/C2′에 있고 둘 다 살아 있는 실패 경로가 있다. **대가는 C4′ = 7/24**: 옛 축이 `no_viewpoint_match`로 거절하던 7샷(고 4·저 3)이 전부 눈높이 플레이트를 받고, 그중 **5샷이 cast**(카드 6장)라 부감·앙각 카드가 눈높이 배경에 합성된다(`camera_angle`은 render-inert가 아니다 — `character_service.py:1556`). **이것은 커버리지 숫자가 아니라 시청 판정이고 Jay의 E2E iteration 5 몫이다.** reason 어휘 7 → **5**(`no_viewpoint_match`·`partial_metadata` 은퇴, 발화 불가). ⚠️ **spec이 틀린 두 곳**: reason 어휘가 움직이는 곳은 넷이 아니라 **다섯**(`tests/domain/test_run_warnings.py`의 파라미터화 등록이 누락돼 있었고 `test_gates.py:258`은 reason이 아니라 warning code 등록이다); 그리고 spec의 Design Notes가 2-경로 입력으로 지목한 `has_person` OR은 이 코퍼스에서 **피연산자 하나로 돈다**(`plate_meta.has_person` 42/42 부재, `label.depicts_person` 42/42 부재) → P2·P3는 `UNDEFINED (0 comparable rows)`이고 **PASS가 아니다**(사전등록 규칙 U). ⚠️ **사전등록 §5와 spec AC가 정면충돌한다** — 전자는 *"(b) 없이는 켜지 않는다"*, 후자는 *"커버리지 충족 시 기본값 True"*. E2E가 플래그 ON 조건부라 **(b)를 얻으려면 먼저 켜야 하는 교착**이고, spec을 따랐으며 그 선택을 `config.py` 주석의 판정과 같은 자리에 적었다. 되돌리는 비용은 한 줄이다. 인계: draft 라벨 14/42(수요 키 3장, 그중 **2장이 실제 배정된다**), `medical-bay/b` 소실점 부재, 플레이트 경로의 런타임 사람 가드 부재, 프롬프트 층 43/43 → 12/43. 근거: `14-8-plate-reuse-shipping/{AXIS-CANDIDATES,PREREGISTRATION,report}.md`. (done)
