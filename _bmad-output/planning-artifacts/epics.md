...
**렌더 전 텍스트 스크리닝 실측**(GPU 0, run `4b35c0ed` 씬 6·8·9 실입력, 구/신 각 3회 × **독립 3회 반복** = 72콜 전부 파싱 성공, `gemini-3.6-flash`): frozen-descriptor 오탐 **구 15/27 rep → 신 0/27 rep**(전 셀·전 반복에서 0). 금지어 지적은 죽지 않았다 — 총 **9/27 → 14/27**, 씬 8은 6/9 → **9/9**. 개체를 `image_prompt`에 다시 주입한 역방향 통제에서 **구 0/9, 신 9/9** — 오탐이 침묵으로 사라진 게 아니라 규칙이 방향을 바꿔 살아 있다(구 프롬프트에는 이 방향의 검사가 **아예 없었다**). 반대 결과 4건 보존: ① `negative_prompt` 삭제 요구는 구 27 rep 중 **1회만 재현**(신 0/27) — 그 절은 강한 전후 신호가 아니라 1회 재현 + 추론으로 정당화된 예방책이다 ② 씬 6 금지어는 신 5/9 vs 구 3/9로 방향만 맞고 n이 작아 불안정 ③ §4 나레이션 진성 모순은 구 9/9 → 신 8/9로 1 rep 하락(다수결 유지, 평탄화하지 않고 기록) ④ `old s9` 오탐이 반복별로 3/3→2/3→0/3으로 흔들렸다 — 단일 3-rep 셀만 봤다면 구 프롬프트가 깨끗해 보여 수정이 무의미해 보일 수 있었다. 오탐 1건의 비용은 배지 오염이 아니라 **그 씬의 pass-2 전면 재작성 1회**(`scenario.py:859` → `_retry_scope`)이므로 경고 2건은 매 런 재작성 2회를 사고 있었다. 증거·재산출: `_bmad-output/implementation-artifacts/14-7-prompt-screening/{report.md,screen_review_prompt.py}`.

DEV MODE 직승격 완료(`migrate_prompts.py --label production --source prompts` → `created: scenario/review`), **런타임 요청 이름으로 확인**(`scenario/review` v10, labels `[production, latest]`, 작업트리와 바이트 동일). ⚠️ 스펙의 검증 URL은 틀렸다 — 이름의 슬래시를 퍼센트 인코딩해야 한다(`prompts/scenario%2Freview`); 생슬래시는 **404**라서 출하 성공을 실패로 오독시킨다. 가드 3건 신설(`test_scenario_chain.py`): 배경 전용 규칙 6핀, 스테일 문장 **부재**의 역방향 핀, 그리고 두 프롬프트의 금지어 목록을 각 파일에서 파싱해 집합 동일성 단정(손으로 베낀 리터럴 두 벌이 실제로 갈라진 사례이므로). 파일: `14-7-scenario-reviewer-recompose-alignment.md`.
