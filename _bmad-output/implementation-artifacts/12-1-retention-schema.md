...
12a. **The critic can score substance, not only delivery.** *(implemented ahead of this story on branch `worktree-critic-fact-grounding`; keep it green rather than re-deriving it)* `critic_step` received only `format_guide` + `scenario_json`, and all five `critic_agent.md` criteria (hook, retention, emotional curve, immersion, ending) judged *delivery*. None asked whether a scene states anything, so filler that correctly applies every mandated immersion technique scored as immersive and passed — the critic structurally could not report the defect Jay observed. `critic_step` now takes `scp_text` as its leading parameter (mirroring `review_step`) and compiles `scp_fact_sheet`; `critic_agent.md` adds criterion 6 (Substance — a scene that could be pasted into any other SCP episode without changing a word has no substance) and criterion 7 (Fidelity — unsupported assertions), plus `retry` rules for two-or-more substance-free scenes and for any unsupported claim, and an instruction to judge those two criteria against the fact sheet only rather than from model SCP knowledge. This is the review-side half of AC 12's writing-side fix: previously `review_step` held the source facts while `writing_step` did not, so review could diagnose what writing could not repair.
