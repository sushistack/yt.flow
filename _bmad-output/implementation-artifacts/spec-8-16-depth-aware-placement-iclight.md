...
- [x] `src/yt_flow/pipeline/nodes/video.py` + `composite_harmonization.py` -- apply the occlusion mask to the card chain when the service returns one -- the overlay has no occlusion concept today, so a foreground object never covers a character.
...
- [x] `scripts/score_composites.py` -- new: Qwen-VL composite QA over rendered frames (does the character look placed in the room, are feet grounded, is lighting consistent), reusing the wiring proven by `label_location_plates.py` -- replaces libcom's composite scorer, and unlike it is calibrated on this project's stylised art.
...
### 2026-08-03 — The two blockers to enabling placement, fixed

- **Ken Burns drift (was "not fixed", the reason the flag stayed off).** `ground_y_expr` in `video.py` mirrors `_zoompan_filter`'s own per-direction `lo/hi/frames` maths as a closed-form time expression, so the anchor tracks the plate's floor instead of a static fraction. Verified against an independent re-derivation of the crop maths for `in-center`, `out-center`, `pan-up`, `pan-down` and `pan-right` at both ends of the clip: delta 0.00000. `overlay` already ran `eval=frame`, so this cost no new filter stage. The contact shadow is a `geq` ellipse with no time terms, so its overlay `y` slides by the *same* expression plus a constant — feet and shadow cannot part company. `_GROUND_Y_MAX` is applied *inside* the expression as well, because the animated value climbs past the static one mid-shot (0.80 reaches 0.845 on a 1.15x push-in) and the motion box would otherwise be violated only on frames nobody sampled.
- **Per-shot depth estimation.** `depth_map_cache_path` is now content-addressed (`<workspace>/cache/depth/<sha256>.png`). `image_node` `shutil.copyfile`s an approved plate into every shot that uses it, so a path key estimated the same 14 plates once *per shot*; the byte key collapses that to one per distinct plate, across runs too, and cannot serve a stale map after a plate is re-rolled in place. `resolve_placements`' in-memory memo keys on the same value.
- **Found while wiring it: a coordinate-system bug neither review caught.** The ground line is measured on the plate, but the Ken Burns chain centre-crops a non-16:9 plate (`scale=1728:-2,crop=1728:972`) before zooming — and the shot generator emits 1344x768 = 1.75, not 1.7778. Feet sat ~6px low on every *generated* background (stock plates are 1920x1080 and were unaffected, which is why the live check on a plate missed it). `frame_fraction` converts plate-height fraction to frame-height fraction; the occlusion mask deliberately keeps the plate-space value, since it is cut in the depth map's own coordinates.
- **`depth_placement_enabled` is now `True`.** Live evidence, real plate + real card + real ffmpeg: the composited feet row measured against the plate's own floor across a 1.15x push-in gives **3.9px max error tracked vs 57.2px static** by the last frame. Ground lines measured across the whole approved library (41 readable plates): strictly ordered `far <= mid <= near` on **41/41**.
- **Two more review findings fixed on the way, both of which only bite with the flag on.** (a) `ground_line` and `occlusion_mask` normalised the depth target against different ranges — the "shared plane" the docstrings promised was two different numbers. `ground_plane` now returns the row *and* the plane value and the mask uses it. (b) `_CARD_HEIGHT_FRAC` was assumed (0.45/0.65/0.82) and wrong by 11-18% of frame height, which the mask resize turns into ~100px of vertical stretch; now 0.406/0.553/0.737, computed from video.py's own `CHAR_MAX_H`/`_DEPTH_SCALE` and re-derived in a test so the copy cannot drift.
- **The ground-line target is normalised against the floor curve's own range, not the raw profile's.** Measured on the library: against the raw range, a plate whose bottom row is already darker than the raw `near` target had every depth hit at index 0 and clamp to 0.98 together — a third of the library collapsed to one depth-independent value, the very defect the running-minimum change was made to remove. Plus `_MIN_GROUND_SPREAD`: 7/41 plates have a near object filling the card's whole column band and genuinely have no floor to read there, so they now degrade to the fallback instead of clamping.
- **`_DEFAULT_GROUND` is measured, not assumed.** far 0.768 / mid 0.848 / near 0.931 — medians over the 41 readable library plates. The old 0.65/0.75/0.85 existed only to make `near` equal the pre-8.16 shadow constant, which is what the review flagged as "far and mid shadows move on unmeasured numbers".
- **`CastMember` now declares `ground_y` and `occlusion_mask`**, and `test_pipeline_imports_no_services` enforces the third AD-1 seam this story is built on (with the four pre-existing crossings allow-listed by name so the list cannot silently grow).
- **`scripts/score_composites.py` landed** (was deferred): Qwen-VL composite QA over rendered frames, three independent judgements (grounded / scale / lighting) rather than one averaged score, frames sampled *across* the clip because a static-anchor regression only shows in its second half. It scored the tracked render 3/3 pass with `grounded: 5` at t=0.04/2.0/3.9 — but it also gave `grounded: 5` to the static render's 57px-off end frame, so its honest role is a gross-failure regression net (mid-air, on a wall, doll-sized), not a drift gauge. That ceiling is recorded in the script's own docstring; pixel measurement stays the authority for "how far off".
