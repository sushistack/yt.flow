...
켜지 않는다: (b) Jay 의 E2E 시청 판정이 AND 조건이고, 여기에 **(c) 릴라이트 결합 수정**이
붙는다(§7 — 증설이 도착하면 한 키가 여러 플레이트를 갖게 되어 그 결함의 발화 빈도가 오른다).
...
3. **이 런에서 이미 발화한다.** §2 의 재산출 배정이 `containment-chamber` 를 씬 3 `/b` ·
...
릴라이트 캐시의 히트율·비용 결정이지 플레이트 선택의 결정이 아니다. **다만 이제 이것은
플래그를 켜기 위한 선행 조건 (c)다**(§3): 부족분을 채우면 한 키가 여러 시점 플레이트를 갖게
되어 이 스토리 자신의 해제 조건이 발화 빈도를 올린다. 오늘 이 결함이 화면에 닿지 않는 유일한
이유는 `stock_plate_substitution_enabled=False` 라는 것뿐이다.
