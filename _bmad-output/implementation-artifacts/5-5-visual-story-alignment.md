...
- [ ] Conditional Phase 2 only after Phase 1 evidence (AC: 9-12, 14) — **candidate now**: Phase 1 A/B evidence (2026-07-04) shows Variant B did not materially improve visual-story alignment, so AC9's trigger condition is arguably met. Awaiting Jay's decision on whether to proceed with Phase 2, iterate on Phase 1 prompts first, or accept current quality. Not started pending that call.
...
- **2026-07-04 — Live A/B + Epic 4 evaluation completed for SCP-096.** Blocker resolved by promoting `scenario/tts_normalize` candidate v1 to `production` (`Langfuse.update_prompt(name="scenario/tts_normalize", version=1, new_labels=["production"])`; SDK's `new_labels` is additive, so v1 now also still carries `candidate`/`latest` — harmless, only one version exists). API server restarted first so the run used the code-review-fixed code (label-gated entity_sheet/story_logline validation, hardened `_scene_role_text`), not the pre-review build.
  ...
  - **Not yet done**: Jay's own gate-review verdict (AC13's other half) by watching both final videos — `workspace/d5fe1f64-7e5c-4a8b-9d9a-c0696266939b/video.mp4` (A) vs `workspace/0161138d-f701-4c31-9591-129fedc0d3e1/video.mp4` (B) — since the automated score alone isn't the full AC13 bar.
  - **Implication for Phase 2 (AC9)**: "Phase 1 A/B results are insufficient" now has direct evidence supporting it, but the loss margin is small and the *cause* is unverified — Jay should decide whether to (a) iterate on Phase 1 prompt wording (e.g. trim the visual_breakdown.md instruction load, or reconsider the forbidden-word list) before spending Phase 2 effort, (b) proceed straight to Phase 2 (SCP Wiki reference image), or (c) accept current quality and close the story. Not decided in this session.
