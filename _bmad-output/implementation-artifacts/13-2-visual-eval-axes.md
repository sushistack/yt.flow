...
### 인계: 8.16 게이트 지표가 체감을 대리하지 못한다 (Story 10.1 AC7, 2026-08-08)

Story 10.1이 run `8a9a288b`를 **video 스테이지만** off/on으로 재렌더해 6샷 프레임 쌍으로 판정한 결과다. 이 스토리가 축을 설계할 때 쓸 1차 보정 데이터이므로 그대로 옮긴다. 근거 파일: `_bmad-output/implementation-artifacts/10-1-live-validation/` (프레임 쌍 6종), 판정 전문은 `10-1-grounding-composite-live-verification.md`의 VERDICT / AC7 절.

**사실:** 8.16은 자체 게이트를 "Ken Burns 추적 오차 3.9px vs 정적 앵커 57.2px"로 통과했다. 10.1은 그 지표가 **정직하다**는 것도 확인했다 — on 상태 카드는 리졸버가 계산한 위치에 픽셀 단위로 정확히 안착했다(`scene_002_S00202` 실측 발끝 y=1018 vs 클램프값 0.9421×1080=1017). 그런데도 Jay의 지적 3("찢어붙인 듯")은 그대로 살아남았다. 즉 **지표가 틀린 게 아니라, 지표가 보는 것과 시청자가 보는 것이 다른 축이다.**

지표가 침묵하는 지점 4가지 — 각각이 축 후보다:

| # | 축 후보 | 8.16 지표가 못 잡는 이유 | 10.1 실측 보정값 |
|---|---|---|---|
| 1 | **플레이트가 접지면을 제공하는가** (이진, 사전 체크) | 바닥이 아예 없는 플레이트도 추적 오차는 동일하게 좋게 나온다 | 슬레이트 6장 중 **2장**이 바닥 없음(`S00202` 벽돌벽, `S00203` top-down 복도). 이 2장은 접지 판정 자체가 성립 불가 |
| 2 | **접지 답이 클램프를 살아남았는가** (비율) | 클램프는 placement **이후**에 걸리고, 추적 오차는 클램프된 값 기준으로 측정되므로 지표에 보이지 않는다 | `_GROUND_Y_MAX=0.9421`(`video.py:1332`)이 near 밴드 **27개 중 23개(85%)** 를 절단, 평균 초과 29.4px. 전체 73카드 중 34.2% |
| 3 | **카드와 플레이트가 빛을 공유하는가** | 미측정 축 | 이 실험을 끝까지 살아남은 결함. `scene_004_S00403`의 거울 바닥이 플레이트 사물은 전부 반사하면서 카드 2장은 반사하지 않음 |
| 4 | **접촉 단서가 읽히는가** | "그림자를 그렸는가 / 따라가는가"는 통과하는데 프레임은 여전히 오려붙인 것으로 읽힌다 | 접촉 그림자는 **존재하고 위치도 정확**(발밑 밴드 off 대비 **+15.7 휘도**, 대조군 +0.05/−0.02, 픽셀 78%가 8레벨 이상 변화). 그럼에도 접촉으로 안 읽힘 |

**1번과 2번은 오늘 바로 계산 가능하다** — 10.1이 이미 뽑은 데이터(체크포인트 + `resolve_placements` 결과)만으로 나온다. GPU도 VLM도 필요 없다. 3·4번이 이 스토리가 만들려는 시각 평가의 본체다.

**방법론 인계 — 배경이 동일한 off/on 프레임 diff는 정밀 계측기다.** video 스테이지만 재실행하면 플레이트의 Ken Burns 체인이 그대로라 **배경이 픽셀 동일**해지고, 두 프레임의 차이는 오직 카드·그림자·마스크뿐이다. 10.1은 이 방법으로 접촉 그림자를 대조군 ±0.05 휘도 수준에서 분리해냈고, **육안 판독이 틀렸던 것을 정정**했다(처음엔 "그림자 없음"으로 봤다). 합성 오버레이 위에 정의되는 축이라면 새 계측 하네스 없이 이 방식으로 보정할 수 있다. 주의: h264 인코드 2개를 diff하는 일반적 함정(`gotcha_h264-diff-cannot-isolate-an-overlay`)은 여기서 대조군 밴드를 같은 프레임 내에 두어 회피했다 — 대조군 없이 절대값만 보면 안 된다.

### References

- [Source: _bmad-output/implementation-artifacts/10-1-grounding-composite-live-verification.md#8.16 Gate Metric vs Perception (AC7)] — 위 인계 절의 원본, 프레임 증거 경로 포함
