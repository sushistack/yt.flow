...
### Sentence Coverage — an Ordered Cover, NOT one shot per sentence

You decide how many shots this scene needs. A shot covers a **range** of consecutive
sentences (`sentence_start`..`sentence_end`), and several consecutive shots may share
one sentence. There are {{sentence_count}} sentences.

The rules are absolute — the subtitles and the video's cuts are derived from this
mapping, so a hole in it loses screen time:

1. **Every sentence 1..{{sentence_count}} is covered by at least one shot.** No gaps.
2. `sentence_start <= sentence_end`, and both are within 1..{{sentence_count}}.
3. **The cover only moves forward.** Shot *n+1*'s range never starts or ends before
   shot *n*'s. List the shots in narration order.
4. **At most {{sentence_count}} shots.** Merging is what pays for splitting — you can
   only afford an extra frame somewhere by giving one up elsewhere.

**MERGE (one shot, several sentences) when a sentence has no visual content of its
own** and would force you to invent an unrelated frame for it. These are the
sentences that produce the worst shots in the finished video:

- a naming or framing line — "이게 에스씨피 재단입니다", "그것이 SCP-049였습니다"
- a bare reaction or qualifier that adds no new place, object or result —
  "아주 협조적으로요", "정말이지 끔찍했습니다"
- a restatement of the sentence before it, or a line whose only content is dialogue
  attribution
- a sentence whose event happens *in the same place, in the same instant* as its
  neighbour — one frame honestly covers both

Give it to the neighbouring shot whose frame the viewer is already looking at while
those words are spoken. Do NOT invent a separate frame for it.

**SPLIT (several shots, one sentence) only when one sentence carries two genuinely
distinct visual beats** — two different places, or a before and an after that cannot
share a frame ("문이 열렸고, 복도 끝의 조명이 하나씩 꺼졌습니다"). Emit the shots in
the order the beats occur; both get the same `sentence_start` and `sentence_end`.

**Default to a merge, not a split.** A scene of {{sentence_count}} sentences that
yields fewer, stronger frames is a better scene. Never emit more shots than sentences.

- For effect/transition-only sentences like `(정적)`, `(pause)`, sound effects with no
  visual content → cover them inside a neighbouring shot's range (preferred), or emit
  an empty `image_prompt` (`""`) for them as before.
...
visual_descriptions:
  ...
  # sentences 2 and 3 share ONE frame — sentence 3 ("이게 에스씨피 재단입니다")
  # names what we are looking at instead of showing anything new
  - image_prompt: |
      ...
    negative_prompt: |
      extra limbs, extra arms, extra fingers, deformed hands, mutated, bad
...
The ranges above run `1..1`, then `2..3` — consecutive, forward-only, and between them
they cover every sentence. That is the whole contract.
...
- [ ] Every sentence from 1 to {{sentence_count}} falls inside at least one shot's `sentence_start`..`sentence_end` range — walk the list and check each number
- [ ] Every shot has `sentence_start <= sentence_end`, both within 1..{{sentence_count}}
- [ ] The ranges only move forward: no shot starts or ends before the shot listed before it
- [ ] Total shot count <= {{sentence_count}} — a split was paid for by a merge
- [ ] Any sentence that names, restates or reacts without showing anything new was MERGED into a neighbour rather than given a frame of its own
...
- [ ] Effect/transition sentences are inside a neighbouring shot's range, or have an empty `image_prompt`
