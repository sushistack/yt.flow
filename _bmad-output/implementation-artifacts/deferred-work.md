...
- **`sound_design_enabled` defaults `True`이지만 CC0 에셋 라이브러리는 아직 소싱되지 않음** — `Settings.sound_design_enabled`(AC7) 기본값이 `True`인데 `data/audio/{bgm,ambient,sfx}`의 12개 실제 파일은 여전히 없음. 실 프로덕션에서 이 상태로 돌리면 매 run마다 `validate_mood_assets`의 fail-fast `FileNotFoundError`로 즉시 실패함. 이미 스토리 자체 Dev Agent Record/Saved Questions에서 Jay가 결정한 사항("review로 마킹, gap은 문서화")이라 코드 리뷰에서 재론하지 않음. **재고 조건**: AC6 에셋 소싱이 실제로 완료되기 전까지는 실 배포 환경에서 `YTFLOW_SOUND_DESIGN_ENABLED=false`를 명시적으로 유지.
