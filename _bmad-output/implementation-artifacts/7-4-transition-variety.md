...
4. **`video_node` builds the tuples.** Each scene tuple carries its mood-resolved transition; each chapter-card tuple carries `"fadeblack"`. **[SUPERSEDED 2026-07-06 — see AC5 amendment below]**
5. ~~**Chapter-card boundaries are always `fadeblack`**~~ **[SUPERSEDED 2026-07-06]** — original AC5 forced every card-touching boundary to `fadeblack`, never a mood-driven type. Reversed per Jay's decision: since `_compose_chapter_card` already color-grades each card by its own upcoming-scene mood (Story 7.2), leaving the *transition into/out of* that same card locked to `fadeblack` was an inconsistency, and — combined with `chapter_cards` defaulting `True` — made the mood-variety feature invisible in the default configuration (every scene after the first is card-adjacent). **New rule:** a card's transition is `resolve_transition(upcoming_mood)` — the same mood value already passed to its color grade — with no adjacency exemption. See Dev Notes "Card-adjacency correctness" section for the pre-amendment rationale (kept for history; no longer the active behavior).
...
### AC5 Amendment (2026-07-06)

Jay decided to reverse the card-adjacency fadeblack lock (see AC4/AC5 above) after the "no-op under default `chapter_cards`" note was raised as a product observation, not a bug, during review. Consequence for two items above:

- The dismissed "feature is a no-op with cards on" note is **resolved**, not just dismissed — cards now carry their own mood-driven transition instead of forcing `fadeblack`.
- The `[Review][Defer] preceded_by_card assumption` item is **resolved by removal** — `preceded_by_card` no longer exists; every scene and card tuple resolves its transition from its own mood, unconditionally.

`test_video_node_card_adjacency_forces_fadeblack` was rewritten to `test_video_node_card_adjacency_uses_mood_transition`, asserting the new mood-driven card behavior instead of the old fadeblack lock.
...
- 2026-07-06: AC5 amendment — removed the card-adjacency fadeblack lock per Jay's decision. Cards now use `resolve_transition(upcoming_mood)`, the same mood already driving their color grade, instead of always `fadeblack`. Rewrote the card-adjacency test accordingly. Full regression green (679 passed, 1 pre-existing skip, 1 unrelated pre-existing flaky test in `test_e2e_stub_run.py` that passes in isolation), ruff clean.
