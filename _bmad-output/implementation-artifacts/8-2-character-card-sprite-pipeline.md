...
- **(2026-07-07, Jay) Frontier-model comparison during curation**: at card-QA time, generate a few of the same cards via a frontier image model (transparent-bg capable, stronger prompt adherence) and compare against local SDXL side-by-side. If clearly better, adopt frontier outputs as style/identity anchors for local generation — library assets are plain files, no pipeline integration needed; runtime generation stays ComfyUI (cost, horror-content filters, seed reproducibility, IPAdapter weight control). Expect asymmetry: entity/creature cards likely hit frontier content filters; human/facility subjects benefit most.
