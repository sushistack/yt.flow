...
7. **Stock/derived reference sourcing.** Given that wiki-fetch is SCP-page-specific (`ScpWikiImageFetch` derives URLs from scp numbers — useless for `STOCK-*`), then stock and derived cards are **by default** generated **without an external reference image**: front angle first via the provider's t2i path (descriptor-driven; the existing `_remove_i2i_input` IPAdapter bypass, `character_image_provider.py:183-232`, invoked deliberately rather than only as error fallback), then the remaining 3 angles i2i **using the just-generated front card as the IPAdapter reference** (self-referencing for cross-angle identity consistency). Expose this as a service method (e.g. `CharacterService.generate_cards_from_descriptor(card_key, pose="standing", anchor_path=None)`) so the seed script, the sitting library (AC12), Story 8.4's on-demand provisioning, and future derived-entity provisioning all share one code path.
   **Optional anchor sourcing (2026-07-07, Jay)**: `seed_stock_cast.py --anchor-search` runs an image search per key (query from the built-in descriptor, e.g. "D-class personnel orange jumpsuit"), downloads top candidates to a review directory, and **stops for human curation** — the operator picks one and re-runs with `--anchor <path>`, which passes it as the front angle's IPAdapter reference (`anchor_path` on the service method; the self-referencing chain after front is unchanged). References are conditioning inputs only, never published assets (same copyright posture as 8.5's style anchors). Depends on Story 5.19 (DDG vqd repair) for the search leg — until it lands, `--anchor <path>` with a manually saved image works standalone. Default (no flags) stays byte-identical to the reference-free path — anchor sourcing is opt-in, not required for AC6/AC13.
...
14. **Look-dev / production split (2026-07-07, Jay — industry-standard protocol).** Given library curation (the human QA stop in Task 5/6), then a **look-dev comparison** is a defined step, not an ad-hoc idea: (a) for 2-3 representative card subjects (one stock human, one facility-adjacent subject, and the entity), the operator produces frontier-model counterparts (transparent-background capable model; produced manually via the model's own UI/API — **no pipeline integration**, the outputs are plain files dropped into a `lookdev/` review directory); (b) Jay judges side-by-side against the local SDXL output; (c) **adoption rule**: if frontier wins for a subject class, its output becomes the **anchor input** for local production of that class (fed through AC7's `anchor_path` — the existing mechanism; frontier defines the look, ComfyUI mass-produces it), or, for one-off library assets that already meet the sprite contract (RGBA, framing, canvas), the frontier file may be adopted **as the card itself**; (d) **production stays local either way**: all runtime/per-run generation remains ComfyUI (cost, horror-content filters, seed reproducibility for 5-14 resume, IPAdapter weight control — the D5 fix depends on it); (e) the outcome (which classes adopted frontier anchors, which stayed pure-local, filter refusals encountered) is recorded in the Dev Agent Record as the look-dev decision — expected asymmetry: entity/creature subjects likely hit frontier content filters, human/facility subjects benefit most. Frontier-adopted files must pass the same `has_alpha` + framing contract as generated cards (no exemptions).
...
5. ~~Frontier-model comparison during curation~~ — **promoted to AC14/Task 8 (2026-07-07)**: the look-dev/production split is now a defined story step with an adoption rule, not an open question.
