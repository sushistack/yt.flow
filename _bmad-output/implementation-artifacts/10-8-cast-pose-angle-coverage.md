...
Status: superseded — 이 초안은 `spec-10-8-cast-pose-angle-coverage.md`의 입력으로 소비됐고,
작업은 그 spec에서 `done`으로 닫혔다(2026-08-16, Jay 시청 판정 `a332ef5`).
여기 적힌 진단 중 하나는 **실증으로 반증됐다**: 「디스크에 65장 있는데 approved가 7장뿐이니
라이브러리를 채우면 된다」는 방향은 시도 후 기각·되돌려졌다(`0301773`). 디스크의 카드가
라벨대로가 아니어서다 — STOCK-d-class의 `sitting` 4장 중 3장이 서 있는 그림이고, 등록하면
같은 그림을 그리면서 `fallback=False`로 보고해 13.1이 없앤 침묵이 되살아난다. 진짜 원인은
추론 토큰이 먹은 예산과 하드코딩된 `angle="front"`였다(`23ebd2b`).
AC3/AC7(라이브러리 채우기)만 후속으로 이월됐다 — 미충족 비용은 40배치 중 3.
