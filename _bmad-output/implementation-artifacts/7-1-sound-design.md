...
- [x] Live validation (AC: 3, 4)
  - [x] Rendered a real 2-scene run (`run_id=7-1-live-validation`) with `YTFLOW_SOUND_DESIGN_ENABLED=true`: scene 1 mood=`dread` (background-only branch, real SCP-096 narration + real background image from a prior run), scene 2 mood=`escalation` (character branch, containment-breach narration + real background+character images), joined via the existing xfade path — exercises both `_compose_scene` branches, a mood-to-mood transition, and real ffmpeg end-to-end (42.16s output, no clipping, no silence, consistent levels across the mood change). Jay confirmed by ear: "잘 되는 것 같음" (sounds good — narration intelligible, ducking, stinger all fine). No constant re-tuning requested.
...
- **2026-07-05: Jay confirmed by ear that all 12 individual asset files now sound fine** ("이제 다 괜찮은듯"). Note this covers the *asset library itself* (AC6), not the full Live Validation task below — nobody has yet run a real 2+ scene render through the pipeline to confirm narration intelligibility, ducking behavior, and stinger timing in context (the mixed/ducked result, not just the raw files in isolation). Leaving that task open.
- **2026-07-05: Live Validation done — story closed.** Rendered a real 2-scene run (`run_id=7-1-live-validation`, output at `workspace/7-1-live-validation/7-1-live-validation/video.mp4`, not committed — a local validation artifact, not a repo asset) reusing real background/character images, real TTS narration audio, and real subtitles from a prior full pipeline run (`workspace/b2dcc3bc-85e5-4ab6-b635-048c98105a2a`). Scene 1: mood=`dread`, background-only branch, SCP-096 setup narration. Scene 2: mood=`escalation`, character branch, containment-breach/evacuation narration. Joined via the existing (untouched) xfade path — exercises both `_compose_scene` branches, a mood-to-mood transition at the join, and the real ffmpeg `-t {duration}` fix under real narration length (not a synthetic sine tone). Automated sanity checks: 42.16s output (≈23.04+19.6-0.5 xfade overlap, correct), no clipping (peak -7.9dB), no silence, consistent mean volume across the mood change (-26.2dB / -26.9dB). Jay's by-ear verdict: "잘 되는 것 같음" — narration intelligible, ducking, stinger all confirmed working. No constant re-tuning requested, so `BGM_VOLUME`/`AMBIENT_VOLUME`/`STINGER_VOLUME`/`SIDECHAIN_*` stay at their original design-doc values. All ACs (1-12) now satisfied; story moved to `done`.
