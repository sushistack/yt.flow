...
| **B1**(§2, 2-경로) | 42행 중 3행 이상 불일치면 FAIL | `label.matches_location == false`가 1/42라 실측은 1행이지만 **3행 이상은 구조적으로 가능**했다(모집단에 상한이 없다) → 발화 가능한 기준 | **NOT VACUOUS**\*, 단 **더 느슨한 잣대 위에서** (아래 §7.2-a) |

**§7.2-a 잣대가 두 개다 — 명시한다.** C1′는 *"오늘 데이터에서 실제로 발화할 수 있는가"*(실측
기준)로 판정했고 B1은 *"3행이 구조적으로 가능했다"*(반사실 기준)로 판정했다. **두 잣대는 같지
않고, 느슨한 쪽이 B1에 적용됐다** — 실측 기준을 B1에도 적용하면 `label.matches_location == false`
1/42는 컷(3행)에 못 미치므로 "오늘 코퍼스에서 이 기준은 FAIL할 수 없었다"가 되어 B1도 VACUOUS로
떨어진다. 여기서 잣대를 하나로 통일하지 **않는** 이유는, 통일하는 방향이 어느 쪽이든 결과를 본 뒤에
기준을 다시 재단하는 일이 되기 때문이다. 그래서 고르는 대신 **B1의 지위가 어느 잣대 위에 서 있는지를
적는다**: B1의 `NOT VACUOUS`는 반사실 기준의 산물이고, 실측 기준으로는 C1′~C3′와 같은 처지다.
따라서 **B1을 근거로 "축의 재현성이 검증됐다"고 주장하지 않는다** — §0의 비맹검 한계(밴드가 필드
전수조사 뒤에 적혔으므로 2.4%는 재현성의 **상한**)와 합치면, B1이 실어 나르는 것은 "반증됐다면
알았을 것"이 아니라 "오늘 반증되지 않았다" 하나다.

**§7.2-b `partial_metadata`는 §3의 C3′ 실패 경로 목록에서 사라진 것이 아니라 은퇴했다.**
§3은 C3′의 살아 있는 실패 경로를 **넷**(`no_metadata` · `partial_metadata` ·
`plate_shows_person` · `no_standing_room`)으로 적었는데 위 표의 C3′ 행은 **셋**만 대조한다.
빠진 하나는 이 스토리가 **reason 어휘에서 제거**했기 때문이다 — 축 ②에는 메타데이터 이후의 매칭
단계가 없으므로 "일부만 측정됐고 그중 매칭 0"이라는 상태가 존재할 수 없고, 발화 불가능한 reason을
남겨 두면 은퇴한 축이 출하된 것처럼 문서에 남는다(`domain/state.py`의 은퇴 기록, `image.py`
`_select_plate` 독스트링). §1~§6은 한 글자도 고치지 않는 것이 §7의 약속이므로, 그 사실을 적을 수
있는 곳은 여기뿐이다. **판정에 영향 없음**: 넷 중 셋이 이미 발화 불가였고 넷째는 존재하지 않는다.
...
- **VACUOUS로 표기된 기준을 근거로 어떤 결정도 세우지 않는다.** 특히
  ...
  (a)∧(b)는 **둘 다 열려 있다**: (a)는 문자 그대로만 충족됐고 그 충족이 반증 불가한 세 기준
  위에 서 있으므로 **증거를 하나도 싣지 않는다**(=충족으로 계상하지 않는다), (b)(Jay 시청
  판정)는 애초에 없다. **(a)를 "사실상 충족"으로 부드럽게 적지 않는다** — 그렇게 적으면 다음
  스토리가 (b) 하나만 채우고 아무 증거도 없는 (a)를 인용하게 된다. (a)를 닫으려면 **실패할 수
  있는 기준**이 필요하고, 그것은 §7.3 마지막 항목대로 다른 코퍼스를 요구한다. §5가 이미
  *"C1′~C3′ 전부 충족돼도 (b) 없이는 켜지 않는다"*고 적었고, 그것이 정본이다.
...
- **공허하지 않은 축은 그대로 남는다**: C4′(7/24, 임계값 없음, Jay 판정 대상). B1(1/42 PASS)도
  남되 **§7.2-a의 단서와 함께** — 그 `NOT VACUOUS`는 반사실 잣대의 산물이고 §0의 비맹검 한계가
  그 위에 겹친다.
