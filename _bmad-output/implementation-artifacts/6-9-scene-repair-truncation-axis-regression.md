...
Status: done  # AC1-5 all closed. AC3: 3-run live multi-trial → axis deltas confirmed VARIANCE (out of scope, 6.8 precedent). AC4: gate re-run ×3, all FAIL by variance → recorded, production NOT promoted. 6-3/6-4 stay blocked by a NEW out-of-scope issue (zero-tolerance gate un-passable under generation noise) — separate decision/story needed.
...
- [x] Task 3: Research how multi-trial statistical comparison is typically done for LLM-judge eval noise (minimum repeat count / confidence interval / paired comparison), then re-run the same candidate against SCP-173 and SCP-096 enough times to apply that method and determine whether the axis deltas are stable (real regression) or noisy (variance). (AC:3) — **DONE (Jay-authorized 2026-07-11): 3 live `--profile promotion` runs on reviewed code (`01ae6cc`). Verdict = VARIANCE** — no `(item,axis)` cell stays negative across the 4 data points (orig+3 runs); negatives wander item/axis run-to-run. See `6-3-6-4-review-metrics-report.md#2026-07-11 Story 6.9 — AC3/AC4 live multi-trial`.
- [x] Task 4: If real regression, evaluate more than one remediation option for the responsible prompt instruction and record why the chosen fix was picked; if variance, document as out of scope (mirroring 6.8's Dev Notes pattern) in this story and in `6-3-6-4-review-metrics-report.md`. (AC:3) — **variance → documented out of scope** in the metrics report + Dev Notes below; no prompt fix chased (6.8 precedent).
- [x] Task 5: Re-run the full 3-item `--profile promotion` gate; promote to `production` on PASS per `docs/PROMPT_POLICY.md`, or record the new FAIL detail. (AC:4) — **DONE: gate re-run ×3, all FAIL (each on a different noise cell), production NOT promoted** (AC4's "still FAILs → record, do not force promotion" branch). Failure detail recorded in the metrics report. 6-3/6-4 stay `in-progress`, now blocked by a newly-identified out-of-scope issue (see Dev Notes → New Blocker).
...
### AC3 — Variance-vs-regression triage (EXECUTED 2026-07-11, Jay-authorized): VARIANCE

Method (per Story 6.8's precedent): a single before/after delta cannot distinguish a real regression from run-to-run generation variance. Applied repeated-trial comparison — re-ran `--profile promotion` (candidate vs production, all 3 golden items, 16k tokens) **N = 3** on the reviewed code (`01ae6cc`), giving 4 data points with the original gate.

**Result: VARIANCE.** No `(item, axis)` cell stays negative across the trials — every cell negative in one run is zero or positive in another (SCP-173 atmosphere: −0.33 → 0 → +1.00 → 0; SCP-173 narrative_coherence: −0.33 → 0 → 0 → −0.67; SCP-096 article_fidelity: −0.33 → +0.33 → +0.67 → 0; SCP-096 narrative_coherence: 0 → +0.67 → −0.33 → −0.33). A real regression from 6.3's reordering / 6.4's serialization would push the *same* cell down consistently; the wandering negatives are full-generation run-to-run noise instead. Per 6.8's precedent → **out of scope, no prompt fix chased.** Full table + reasoning in `6-3-6-4-review-metrics-report.md#2026-07-11 Story 6.9 — AC3/AC4 live multi-trial`.

### New Blocker (out of 6.9 scope) — why 6-3/6-4 are still not promotable

AC4's gate re-ran 3× and FAILed every time, but never for a reproducible reason (a different noise cell each run). Root cause: the promotion gate is **zero-tolerance** (any single negative delta = FAIL — a deliberate Story 6.6 policy) over a 3-item × 3-axis = 9-cell comparison, and full-generation variance reliably drives *some* cell slightly negative on every run. So a candidate that is statistically equivalent to production still FAILs by chance every time. This is a policy/measurement tension, explicitly **out of 6.9's scope** (Out Of Scope forbids changing `PROMPT_POLICY.md`'s zero-tolerance rule). Unblocking 6-3/6-4 needs a **separate decision/story**: a statistical gate (mean delta over N runs ≥ 0, or a tolerance band within judge granularity) or a median/best-of-N promotion criterion. Also surfaced: SCP-049's scoped `writing_scene_repair` is intermittently fragile (truncation *and* a `scene coverage mismatch` across the runs) — a separate robustness follow-up; note the coverage mismatch correctly failed the run (not a truncation), live-confirming 6.9's narrow recovery.
...
- ✅ AC3: 3-run live multi-trial executed (Jay-authorized 2026-07-11) → axis deltas confirmed **VARIANCE**, documented out of scope per 6.8 precedent (metrics report + Dev Notes → AC3).
- ✅ AC4: gate re-run ×3, all FAIL (different noise cell each run) → recorded, `production` NOT promoted (AC4's FAIL→record branch). 6-3/6-4 stay blocked by a new out-of-scope issue (zero-tolerance gate vs generation noise) — separate decision/story flagged.
- Full suite: **1247 passed, 1 skipped**; ruff clean (post-review-fix, `01ae6cc`).
...
- 2026-07-11: **Story CLOSED (done).** AC3/AC4 executed live (Jay-authorized): 3× `--profile promotion` on reviewed code `01ae6cc`. AC3 → axis deltas confirmed **VARIANCE** (no cell stays negative across 4 data points; wandering negatives = generation noise) → out of scope per 6.8 precedent. AC4 → gate FAILed all 3 runs (different noise cell each), `production` NOT promoted (FAIL→record branch), 6-3/6-4 stay `in-progress`. Two new out-of-scope findings recorded: (1) zero-tolerance gate is structurally un-passable under generation variance → needs a separate policy/statistical-gate decision to unblock 6-3/6-4; (2) SCP-049 scoped-repair intermittently fragile (truncation + coverage-mismatch), the mismatch live-confirmed 6.9's narrow recovery by correctly failing the run. See `6-3-6-4-review-metrics-report.md#2026-07-11 Story 6.9 — AC3/AC4 live multi-trial`.
