...
- Task 2 golden-set gate (2026-07-08 rerun): `uv run python scripts/eval_prompts.py --label candidate --baseline production` → `FAIL`; `scenario/cast_decision` now fetches for both `production` and `candidate`, but the gate fails on quality: `SCP-049` item failure, `SCP-096` item failure, `SCP-173` article_fidelity regression (-1.00) with total +0.00. Task 2 remains open.
