...
# returns, `domain/state.py`'s documented list, `domain/warnings.py`'s prose,
# `14-1-approved-plate-sets/replay_coverage.py`'s histogram and THIS list) — Story 14.8's
# spec named four and the fourth it named (`tests/pipeline/test_gates.py:258`) is a
# warning CODE row, not a reason. That census result is recorded here so the next edit
