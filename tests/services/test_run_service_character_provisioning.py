...
async def test_search_failure_is_non_fatal(monkeypatch, tmp_path):
    ...
    # AD-10 fallback: no angle paths were ever set, so select_character_angles's own
    # "no angle paths" branch (character_service.py) degrades to the segmentation
    # cutout exactly as it would with no CharacterModel at all.
    character = _get_character("SCP-096")
    assert character is not None
